...
### Covered/Not_Covered
Bed = pd.read_csv("KAPA_HyperExome_hg38.bed", sep = '\t', header = None, error_bad_lines=False)
...
Genes_13 = pd.read_csv("13_Genes_Key_Variants_Coordinates.csv", sep="\t")
vcf = pd.read_csv("Input_final_DP.vcf".format(sys.argv[1], sys.argv[1]), sep="\t", comment="#")
...
Genes_vcf.to_csv("Output_final_DP_Cond1.csv".format(sys.argv[1], sys.argv[1]), sep="\t", index=False)
print("Condition1 is done and placed output")

### Condition2: Single shared file
print("### Condition2: Single shared file")
Shared_single = pd.read_csv("13_genes_single_shared_Updated.csv", sep="\t")
vcf = pd.read_csv("Input_final_DP.vcf", sys.argv[1]), sep="\t", comment="#")
vcf.columns = ['CHROM', 'POS', 'rsID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', 'SAMPLE']
...
Shared_vcf.to_csv("Output_final_DP_Cond2.csv".format(sys.argv[1], sys.argv[1]), sep="\t", index=False)
