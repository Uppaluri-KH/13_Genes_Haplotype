import pandas as pd
import sys
df = pd.read_excel("1232_PGx_unique_star_alleles_Function_Evidence.xlsx")
df = df[['CHROM', 'POS', 'REF', 'ALT', 'rsID', 'Gene', 'Gene_star_allele_updated']]

data = pd.read_excel("Input_snp_pos_distance.xlsx".format(sys.argv[1], sys.argv[1]))
...
data.to_excel("Output_snp.xlsx", index = False)
