import sys
import pandas as pd
variants = pd.read_csv("Input_final_DP.vcf", comment= '#', sep = '\t', header=None, low_memory=False)
variants.columns = ['CHROM', 'POS', 'rsID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', 'SAMPLE']
df = pd.read_excel("13genes_coordinates_haplotypes.xlsx", header = 0)
...
df3 = pd.read_excel("main_new_data.xlsx")
...
final.to_excel("Output_snp_pos_distance.xlsx", index = False)
